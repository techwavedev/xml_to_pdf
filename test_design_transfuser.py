...
def transfuse_docx_design_with_json(docx_input_path: str, docx_output_path: str, json_data: dict, photo_path: str = "assets/photo.png"):
    """
    Extrai o DESIGN (estilos, tabelas, fontes, cores) de docx_input_path e constrói o CONTEÚDO
    100% exclusivamente a partir de json_data.
    """
    ...
    # Hashes SHA-256 (primeiros 12 chars) das logos do SprintCV para limpeza
        ...
        with zipfile.ZipFile(temp_zip, 'w') as jout:
            for item in jin.infolist():
                ...
                if fname.startswith('word/media/'):
                    ...
                    # 1. Substituição de contatos e cabeçalho do candidato
                    doc_str = re.sub(r'(?i)\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b', email_xml, doc_str)
                    ...
                    # 2. Substituição do Nome e Cargo no cabeçalho
                    ...
                    # 3. Substituição do Resumo Profissional (Summary)
                    ...
                    # 4. Substituição das Habilidades Técnicas / Top Technologies
                    if techs_xml:
                        ...
                        doc_str = re.sub(r'(?i)Amazon Bedrock.*?SonicWall', techs_xml, doc_str)

                    # 5. Substituição de Idiomas
                    if languages:
                        lang_list = [f"{l.get('name') or l.get('language', '')} ({l.get('proficiency') or l.get('level', '')})" for l in languages]
                        lang_xml = html.escape(" | ".join(lang_list), quote=False)
                        doc_str = re.sub(r'English \(C2\), Portuguese \(Native\)', lang_xml, doc_str)
    ...
    if os.path.exists(temp_zip):
        ...
        print(f"[✓] Layout do DOCX preservado 100% e conteúdo atualizado a partir do JSON em: {docx_output_path}")
        return True
    return False
...
if __name__ == "__main__":
    with open("sample_cv.json") as f:
        jdata = json.load(f)
    transfuse_docx_design_with_json("samples/expert.docx", "output/transfused_expert.docx", jdata)
