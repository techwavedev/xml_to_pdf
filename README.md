...
Sistema completo em Python para gerar Currículos/CVs em PDF estilizados a partir de uma base única de dados em JSON, com suporte a **múltiplos templates gráficos** e **incorporação automática de metadados ATS XML** baseados nas diretrizes da SprintCV.
...
2. **Templates Gráficos Flexíveis (`templates/`)**: O mesmo arquivo JSON pode ser renderizado em diferentes layouts gráficos HTML/CSS (`sprintcv_docx`, `sprintcv`, `modern`, `classic`, `tech_dark`).
3. **Conversão Automática para HR-XML**: O sistema converte o JSON automaticamente para o padrão internacional HR-XML 3.0.
4. **Otimização para ATS (Applicant Tracking Systems)**: O XML e metadados XMP/Info são injetados diretamente dentro do PDF gerado (de forma limpa e invisível para o leitor humano).
...
- **SprintCV DOCX Official** (Baseado exatamente no documento `.docx` da pasta `Samples/`):
  ```bash
  python3 cv_builder.py --json meu_cv.json --template sprintcv_docx
  ```
- **SprintCV Modern** (Layout SprintCV simplificado):
  ```bash
  python3 cv_builder.py --json meu_cv.json --template sprintcv
  ```
...
O PDF final será salvo automaticamente em `output/` (ex: `output/John_sprintcv_docx_ats.pdf`).
...
- **[templates/](file:///Users/elton/Library/CloudStorage/SynologyDrive-m1/code/xml_to_pdf/templates)**: Templates HTML/CSS (`sprintcv_docx.html`, `sprintcv.html`, `modern.html`, `classic.html`, `tech_dark.html`).
- **[Samples/](file:///Users/elton/Library/CloudStorage/SynologyDrive-m1/code/xml_to_pdf/Samples)**: Pasta privada para seus arquivos de exemplo/templates locais (ignorada no Git).
